...
module WIDEQ
  ...
  def self.gateway_info
    return lgedm_post(GATEWAY_URL, data: {countryCode: COUNTRY, langCode: LANGUAGE})
  end
  ...
  def self.parse_oauth_callback url
    ...
    return uri.query_values['access_token'], uri.query_values['refresh_token']
  end
  ...
  def self.login api_root, access_token
    ...
    return lgedm_post url, data: data
  end
  ...
  class Gateway
    ...
    def self.discover
      ...
      return Gateway.new gw['empUri'], gw['thinqUri'], gw['oauthUri']
    end

    def oauth_url
      return WIDEQ::oauth_url @auth_base
    end
  end
  ...
  class Auth
    ...
    def self.from_url gateway, url
      ...
      return Auth.new gateway, access_token, refresh_token
    end
    ...
    def start_session
      ...
      return Session.new(self, session_id), WIDEQ::get_list(session_info, 'item')
    end
    ...
    def refresh
      ...
      return Auth.new(@gateway, new_access_token, @refresh_token)
    end
  end
  ...
  class Session
    ...
    def post path, data=nil
      ...
      url.chop if url[-1] == '/'
      url += '/' + path
      return WIDEQ::lgedm_post(url, data: data, access_token: @auth.access_token, session_id: @session_id)
    end

    ## Get a list of devices associated with the user's account.
    # Return a list of dicts with information about the devices.
    def get_devices
      return WIDEQ::get_list(post('device/deviceList'), 'item')
    end
    ...
    def monitor_poll device_id, work_id
      ...
      code = (res.has_key? 'returnCode') ? res['returnCode'] : nil # returnCode can be missing.
      if code != '0000'
        raise MonitorError.new(device_id, code)
      end
      # The return data may or may not be present, depending on the
      # monitoring task status.
      if res.has_key? 'returnData'
  ...
  class Monitor
    ...
    def poll
      begin
        return @session.monitor_poll @device_id, @work_id
      rescue MonitorError
    ...
    def poll_json
      data = poll
      return data.nil? ? nil : decode_json(data)
    end
  end
  ...
  class Client
    ...
    def devices
      ...
      return @_devices.map { |d| DeviceInfo.new d }
    end
    ...
    def model_info device
      ...
      if not @_model_info.has_key? url
        @_model_info[url] = device.load_model_info
      end
      return ModelInfo.new @_model_info[url]
    end
    ...
    def self.load state
      ...
      if state.has_key? 'session'
        session = Session.new auth, state['session']
      end
    ...
    def dump
      ...
      if @_session
        out['session'] = @_session.session_id
      end
  ...
  class DeviceInfo
    ...
    # The kind of device
    def type
      return DEVICE_TYPE.key(@data['deviceType'])
    end

    ##
    # Load JSON data describing the model's capabilities.
    def load_model_info
      return JSON::parse(RestClient.get(model_info_url))
    end
  end
  ...
  class ModelInfo
    ...
    def value name
      begin
        ...
        if type.casecmp? 'enum'
          ...
          return EnumValue.new d['option']
        elsif type.casecmp? 'range'
          $logger.debug "#{name} is a range"
          return RangeValue.new d['option']['min'], d['option']['max'] #, d['option']['step']
    ...
    def reference_name key, val
      ...
      return '-' unless reference.has_key? val

      comment = reference[val]['_comment']
      return comment if comment
      return reference[val]['label']
    end

    ##
    # Check that type of monitoring is BINARY(BYTE).
    def binary_monitor_data
      return @data['Monitoring']['type'] == 'BINARY(BYTE)'
    end
    ...
    def decode_monitor_binary data
      decoded = {}
      @data['Monitoring']['protocol'].each { |item|
        key = item['value']
        value = 0
        data[item['startByte']..(item['startByte'] + item['length'] - 1)].unpack('C*').each { |v|
          value = (value << 8) + v
        }
        decoded[key] = value
      }
    ...
    def decode_monitor_json data
      return JSON::parse(data.decode('utf8'))
    end
    ...
    def decode_monitor data
      if binary_monitor_data
        return decode_monitor_binary(data)
      else
        return decode_monitor_json(data)
      end
    end
  end
  ...
  class Device
    ...
    def _get_config key
      ...
      return JSON.parse(base64.b64decode(data).decode('utf8'))
    end
